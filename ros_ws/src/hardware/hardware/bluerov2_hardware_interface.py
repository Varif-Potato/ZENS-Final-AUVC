...
class BlueROV2HardwareInterface(Node):
    ...
    def _handle_pressure(self, msg):
        ...
        pressure_msg.fluid_pressure = msg.press_abs * 100.0
        self.pressure_pub.publish(pressure_msg)

        temperature_msg = Temperature()
        temperature_msg.header.stamp = self.get_clock().now().to_msg()
        temperature_msg.temperature = msg.temperature / 100.0
    ...
    def manual_control_callback(self, msg: ManualControl):
        self._x = int(msg.x * 10)
        self._y = int(msg.y * 10)
        self._z = int((msg.z * 10) / 2 + 500)
        self._r = int(msg.r * 10)
